<?xml version="1.0" encoding="UTF-8"?><!--(c) 2002, 2014 LANSA--><!--XHTML Standard Scripts--><!--$Workfile:: std_script.xsl $--><!--$UTCDate:: 2014-01-24 03:54:51Z $--><!--$Revision:: 34 $--><xsl:transform version="1.0" exclude-result-prefixes="lxml wd" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lxml="http://www.lansa.com/2002/XML/Runtime-Data" xmlns:wd="http://www.lansa.com/2002/XSL/Weblet-Design" xmlns="http://www.w3.org/1999/xhtml"><xsl:import href="std_keys.xsl"></xsl:import><xsl:import href="std_variables.xsl"></xsl:import><xsl:template name="script"><xsl:param name="javascript_files" select="''"></xsl:param><xsl:param name="javascript_files_charset" select="$lweb_extrafiles_charset"></xsl:param><xsl:param name="trap_script_errors" select="false()"></xsl:param><xsl:param name="jQueryNoConflict" select="false()"></xsl:param><xsl:if test="not($lweb_design_mode)"><script type="text/javascript"><xsl:text disable-output-escaping="yes">//&lt;![CDATA[
</xsl:text>
...
var g_lxmlDecSeparator = "<xsl:value-of select="$lweb_context/lxml:dec-separator"></xsl:value-of>";
