/*!
	...
	$Revision:: 109                         $
*/
if(document.all&&!document.getElementById){document.prototype.getElementById=function(id){return document.all[id];};}(function(){var dbg=window.g_debug||/[?&+]debug=(yes|y)/i.test(document.location.href)||false;var count=window.loglimit||299;window.loglimit=count;if(!window.console){console={};}var common="info,error,log,warn";
var more="debug,time,timeEnd,assert,count,trace,dir,dirxml,group,groupEnd,groupCollapsed,exception";var fixIE=function(){var calls=common.split(",");for(var i=0;i<calls.length;i++){var m=calls[i];var n="_"+calls[i];console[n]=console[m];console[m]=(function(){var type=n;return function(){count--;if(count==0){console._log("***LOG LIMIT OF "+loglimit+" HAS BEEN REACHED***");}if(count<1){return;}if(console[type]){console[type](Array.prototype.slice.call(arguments).join(" "));}};})();}try{console.clear();
}catch(e){}};var fixMobile=function(){var calls=common.split(",");for(var i=0;i<calls.length;i++){(function(){var m=calls[i];var n="_"+calls[i];console[n]=console[m];console[m]=function(){console[n](Array.prototype.slice.call(arguments).join(" "));};})();}};var hideCalls=function(str){var calls=str.split(",");for(var i=0;i<calls.length;i++){console[calls[i]]=function(){};}};var tweak=function(){var calls=more.split(",");for(var i=0;i<calls.length;i++){if(!console[calls[i]]){console[calls[i]]=function(){};
}}};var ua=window.navigator.userAgent;if(dbg&&/Trident/.test(ua)){fixIE();hideCalls(more);}else{if(dbg&&/WebKit|iPad|iPhone/.test(ua)){fixMobile();tweak();}else{if((/IE/.test(ua)&&!/Trident/.test(ua))||!dbg||!window.console){hideCalls(more+","+common);}else{tweak();}}}})();var debug=function(){console.log.apply(console,arguments);};var jslog={debug:debug,info:debug,warning:debug,error:debug,text:debug};console.log("JavaScript debugging enabled");String.prototype.zeropad=function(l){var s="",i=this.length;
while(i++<l){s+="0";}return s+this;};String.prototype.lpad=function(len,pad){pad=(!pad)?" ":pad.substring(0,1);return(len-=this.length)>0?(new Array(len+1).join(pad)+this):""+this;};String.prototype.rpad=function(len,pad){pad=(!pad)?" ":pad.substring(0,1);return(len-=this.length)>0?(this+new Array(len+1).join(pad)):""+this;};String.prototype.ltrim=function(){return this.replace(/^\s+/g,"");};String.prototype.rtrim=function(){return this.replace(/\s+$/g,"");};String.prototype.trim=function(){return this.replace(/^\s+|\s+$/g,"");
};Number.prototype.zeropad=function(l){return this.toString().zeropad(l);};if(!Array.prototype.indexOf){Array.prototype.indexOf=function(elt){var len=this.length;var from=Number(arguments[1])||0;from=(from<0)?Math.ceil(from):Math.floor(from);if(from<0){from+=len;}for(;from<len;from++){if(from in this&&this[from]===elt){return from;}}return -1;};}if(!Array.prototype.forEach){Array.prototype.forEach=function(fun){var len=this.length;if(typeof fun!="function"){throw new TypeError();}var thisp=arguments[1];
for(var i=0;i<len;i++){if(i in this){fun.call(thisp,this[i],i,this);}}};}Date.prototype.setISO8601=function(dateStr){if(dateStr.length==0){this.sqlNull=true;return;}var regexp=/(\d\d\d\d)(?:-?(\d\d)(?:-?(\d\d)(?:[T ](\d\d)(?::?(\d\d)(?::?(\d\d)(?:\.(\d+))?)?)?(?:Z|(?:([-+])(\d\d)(?::?(\d\d))?)?)?)?)?)?/;var d=dateStr.match(regexp);var offset=0;if(!d||!d[1]){return null;}var date=new Date(d[1],0,1);if(d[2]){date.setMonth(d[2]-1);}if(d[3]){date.setDate(d[3]);}if(d[4]){date.setHours(d[4]);}if(d[5]){date.setMinutes(d[5]);
}if(d[6]){date.setSeconds(d[6]);}if(d[7]){date.setMilliseconds(Number("0."+d[7])*1000);}if(d[8]){offset=(Number(d[9])*60)+Number(d[10]);offset*=((d[8]=="-")?1:-1);}offset-=date.getTimezoneOffset();time=(Number(date)+(offset*60000));this.setTime(Number(time));this.sqlNull=false;};Date.prototype.toISO8601String=function(includeTime,useLocalTime){if(includeTime==undefined){var includeTime=true;}if(useLocalTime==undefined){var useLocalTime=true;}var date=this;var nY=0,nM=0,nD=0,nH=0,nMin=0,nS=0,nMs=0,nOffset=0;
if(useLocalTime){nY=date.getFullYear();nM=date.getMonth()+1;nD=date.getDate();nH=date.getHours();nMin=date.getMinutes();nS=date.getSeconds();nMs=date.getMilliseconds();nOffset=-date.getTimezoneOffset();}else{nY=date.getUTCFullYear();nM=date.getUTCMonth()+1;nD=date.getUTCDate();nH=date.getUTCHours();nMin=date.getUTCMinutes();nS=date.getUTCSeconds();nMs=date.getUTCMilliseconds();nOffset=0;}var str=nY+"-"+nM.zeropad(2)+"-"+nD.zeropad(2);if(includeTime){str+="T"+nH.zeropad(2)+":"+nMin.zeropad(2)+":"+nS.zeropad(2);
if(nMs>0){str+="."+nMs.zeropad(3);}}if(nOffset==0){str+="Z";}else{if(nOffset>=0){str+="+";}str+=Math.round(nOffset/60).zeropad(2)+":"+(nOffset%60).zeropad(2);}return str;};Date.prototype.arrayMstringDays=function(){return stdGetMTextMessageTextArr("DaysOfWeek");};Date.prototype.arrayMstringMonths=function(){return stdGetMTextMessageTextArr("Months");};Date.prototype.toFormattedString=function(f){var d=this;return f.replace(/(YYYY|YY|Y|MMMM|MM|M|DDD|DD|D|HH|H|hh|h|mm|m|sss|ss|s|t)/g,function($1){switch($1){case"YYYY":return d.getFullYear();
case"YY":return(d.getFullYear()%100).zeropad(2);case"MMMM":return d.arrayMstringMonths()[d.getMonth()];case"MM":return(d.getMonth()+1).zeropad(2);case"M":return(d.getMonth()+1);case"DDD":return d.arrayMstringDays()[(d.getDay()+6)%7];case"DD":return d.getDate().zeropad(2);case"D":return d.getDate();case"HH":return d.getHours().zeropad(2);case"H":return d.getHours();case"hh":return((h=d.getHours()%12)?h:12).zeropad(2);case"h":return((h=d.getHours()%12)?h:12);case"mm":return d.getMinutes().zeropad(2);
case"m":return d.getMinutes();case"ss":return d.getSeconds().zeropad(2);case"s":return d.getSeconds();case"sss":return d.getMilliseconds().zeropad(3);case"t":return d.getHours()<12?"A.M.":"P.M.";}});};Lstd=window.Lstd||{};Lstd.bootstrap=function(){var onLoadFunc=function(){Lstd.init();};if(window.jQuery){if(window.g_lweb_jQueryNoConflict){jQuery.noConflict();}jQuery(document).ready(onLoadFunc);}else{if(window.addEventListener){window.addEventListener("load",onLoadFunc,false);}else{if(window.attachEvent){window.attachEvent("onload",onLoadFunc);
}}}};Lstd.init=function(){var w,wn,weblets=this.Weblets._weblets;for(wn in weblets){w=weblets[wn];if(w.options.init){w.options.init(w.id,w.data);}}if(!Lstd.Utils.testAttribute("input","autofocus")){var ae;try{ae=document.activeElement;}catch(e){}if((!ae||ae.nodeName=="BODY")){var focusDone=false;for(var i=0,f;f=document.forms[i];i++){for(var j=0,e;e=f.elements[j];j++){if(e.getAttribute("autofocus")){try{e.focus();focusDone=true;break;}catch(err){}}}if(focusDone){break;}}}}};Lstd.Utils={strargs:function(str,args){if(args==null||typeof(args)=="undefined"){args=[];
...
fld=resExtraFields[i];i++){insertHiddenField(f,fld.name,fld.value);}},check4VF_WAMEVENT:function(f,elem){var vf_wamevent=elem.getAttribute("data-vf-wamevent");if(vf_wamevent){insertHiddenField(f,"VF_WAMEVENT",vf_wamevent);}}};Lstd.Lists={};Lstd.registerLists=function(lists){if(Lstd.Json){for(var i=0,cur;cur=lists[i];i++){try{var lst=Lstd.Json.factory(cur);Lstd.Lists[lst.name()]=lst;}catch(e){console.error("Lstd.registerLists() [list index: "+i+"]: "+e);}}}};Lstd.Weblets={_weblets:{},registerWeblet:function(id,options,data){if(id==""){console.error("A weblet must have an ID");
...
}if(WRName!=""){Lstd.Fields.check4VF_WAMEVENT(f,elem);submitForm(f,WAMName,WRName,target,protocol);}}function std_DD_ListBoxOnKeyDown(oForm,e){if(!e){if(window.event){e=window.event;}else{return;}}if(e.keyCode==13){setTimeout(function(){_HandleDefaultSubmit(oForm);},0);}}function stdListBoxInsertMultiSelectList(f,elem,bSubmitTagFields,sTagFld1,sTagFld2,sTagFld3,bAllowMultiSel,multiSelListName,multiSelCodeField){var dataArray=new Array();var columns=new Array();columns.push(multiSelCodeField);if(bSubmitTagFields){if(sTagFld1!=""){columns.push(sTagFld1);
}if(sTagFld2!=""){columns.push(sTagFld2);}if(sTagFld3!=""){columns.push(sTagFld3);}}var options=elem.options;if(options!=null){for(var i=0,o;o=options[i];i++){if(o.selected){var newRow=new Array();newRow.push(o.value);if(bSubmitTagFields){if(sTagFld1!=""){newRow.push(o.getAttribute("tag_"+sTagFld1));}if(sTagFld2!=""){newRow.push(o.getAttribute("tag_"+sTagFld2));}if(sTagFld3!=""){newRow.push(o.getAttribute("tag_"+sTagFld3));}}dataArray.push(newRow);}}}stdInsertHiddenList(f,multiSelListName,columns,dataArray);
}function stdListBoxInit(name,formname,bSubmitTagFields,sTagFld1,sTagFld2,sTagFld3,bAllowMultiSel,multiSelListName,multiSelCodeField,value,selsize,defaultOption){var listBoxInfo=new Object();listBoxInfo.name=name;listBoxInfo.formname=formname;listBoxInfo.bSubmitTagFields=bSubmitTagFields;listBoxInfo.sTagFld1=sTagFld1;listBoxInfo.sTagFld2=sTagFld2;listBoxInfo.sTagFld3=sTagFld3;listBoxInfo.bAllowMultiSel=bAllowMultiSel;listBoxInfo.multiSelListName=multiSelListName;listBoxInfo.multiSelCodeField=multiSelCodeField;
if(!g_LANSAObjects.typeExists("std_listbox")){addEvent(window,"load",stdListboxWindowLoaded);}g_LANSAObjects.addObject(listBoxInfo,"std_listbox",name);if(defaultOption){stdListboxSelDft(name,value,selsize,defaultOption);}}function stdListboxWindowLoaded(){var listboxes=g_LANSAObjects.getObjectsByType("std_listbox");for(var i=0,lbi;lbi=listboxes[i];i++){var lb=document.getElementById(lbi.name);if(lb){var f=document.forms[lbi.formname];if(!f){f=lb.form;}if(f){stdListBoxInsertMultiSelectList(f,lb,lbi.bSubmitTagFields,lbi.sTagFld1,lbi.sTagFld2,lbi.sTagFld3,lbi.bAllowMultiSel,lbi.multiSelListName,lbi.multiSelCodeField);
}}}}function stdListboxSelectedOptions(elem){var opts=elem.options;var selopts=new Array();for(var i=0,o;o=opts[i];i++){if(o.selected){selopts.push(o);}}return selopts;}function stdListboxSelDft(name,value,selsize,defaultOption){var elem=document.getElementById(name);if(elem){var selopts=stdListboxSelectedOptions(elem);if((selopts.length==0)||((selsize==1)&&(value!=selopts[0].value))){var opts=elem.options;for(var i=0,o;o=opts[i];i++){if(o.value==defaultOption){o.selected=true;break;}}}}}function stdBannerClicked(e,elem,url,WAMName,WRName,formName,protocol,target){if(!e){if(window.event){e=window.event;
}else{return;}}if((url=="")&&(WRName!="")){var f=(!formName)?getParentForm(elem):createStdHiddenForm(WAMName,WRName,formName,protocol,target);if(f!=null){createReentryFields(f,elem.parentNode);submitForm(f,WAMName,WRName,target,protocol);}cancelEvent(e);return false;}else{if((url!="")&&(target!="")){window.open(url,target);return false;}}return true;}function stdPrompterInitIFrame(){var iframe=g_LANSAObjects.getObjectById("stdPrompterFrame");if(iframe==null){iframe=document.createElement("iframe");
iframe.id=(window.name=="")?"stdPrompterFrame":window.name+".stdPrompterFrame";iframe.name=iframe.id;iframe.src="";iframe.setAttribute("frameborder","0");iframe.style.position="absolute";iframe.style.visibility="hidden";iframe.style.width="1px";iframe.style.height="1px";iframe.style.zIndex=99999;iframe.getContentDoc=function(){try{return(this.contentDocument)?this.contentDocument:iframe.contentWindow.document;}catch(err){console.error("JavaScript error in iframe.getContentDoc: "+err.toString());return null;
}};iframe.getContentDocLocation=function(){var doc=this.getContentDoc();if(doc){try{return doc.location;}catch(err){console.error("JavaScript error in iframe.getContentDocLocation: "+err.toString());return null;}}};g_LANSAObjects.addObject(iframe,"stdPrompterFrame","stdPrompterFrame");iframe.onload=stdPrompterIFrameLoaded;if(!window.opera){iframe.onreadystatechange=stdPrompterIFrameRSChange;}document.body.appendChild(iframe);if(iframe.contentWindow){iframe.contentWindow.name=iframe.id;}}if(!iframe.__theBtn){iframe.__theBtn=null;
}return iframe;}function stdPrompterBtnClick(theBtn,formname,url,WAMName,WRName,protocol,width,height,autoResize,borderStyle,borderWidth,className,constrainSize){if(theBtn.className=="disabled"){return;}var iframe=stdPrompterInitIFrame();if((iframe.__theBtn!=theBtn)||(iframe.style.visibility=="hidden")){if(iframe.__closeMeTO){clearTimeout(iframe.__closeMeTO);iframe.__closeMeTO=null;}iframe.style.visibility="hidden";iframe.windowRight=document.body.scrollLeft+document.body.clientWidth;iframe.windowBottom=document.body.scrollTop+document.body.clientHeight;
...
if(tf){tf.value=f.value;if(!focusField){focusField=tf;}if(tf.dispatchEvent){var evtObj=document.createEvent("HTMLEvents");evtObj.initEvent("change",false,true);tf.dispatchEvent(evtObj);}else{if(tf.fireEvent){tf.fireEvent("onchange");}else{tf.onchange();}}}}}if(focusField){try{focusField.focus();focusField.select();}catch(e){}}}function stdPrompterGetTargetField(iframe,fieldName,formName){var fields=document.getElementsByName(fieldName);var f=null;var pf=iframe.__theBtn.form;for(var i=0;i<fields.length;
i++){if(i==0){f=fields[0];}if(formName&&fields[i].form.name==formName){f=fields[i];break;}if(fields[i].form==pf){f=fields[i];}}return f;}function stdPrompterSetFrameSize(iframe){if(!iframe){iframe=g_LANSAObjects.getObjectById("stdPrompterFrame");}var doc=iframe.getContentDoc();if(doc&&doc.body){doc.body.style["margin"]="0px";var theBtn=iframe.__theBtn;var btnPos={left:getAbsoluteLeft(theBtn),top:getAbsoluteTop(theBtn)};btnPos.right=btnPos.left+theBtn.offsetWidth;btnPos.bottom=btnPos.top+theBtn.offsetHeight;
if(iframe.__autoResize){var w,h;iframe.style.width=iframe.__width;iframe.style.height=iframe.__height;if((document.compatMode=="BackCompat")||!Lstd.Utils.boxModel()||(navigator.userAgent.indexOf("MSIE 7")!=-1)){w=doc.body.scrollWidth+(iframe.offsetWidth-iframe.clientWidth)+2;h=doc.body.scrollHeight+(iframe.offsetHeight-iframe.clientHeight)+2;}else{w=doc.documentElement.scrollWidth;h=doc.documentElement.scrollHeight;doc.documentElement.style["overflow"]="hidden";}iframe.style.width=w.toString()+"px";
iframe.style.height=h.toString()+"px";}var newSize={width:iframe.offsetWidth,height:iframe.offsetHeight};var space={right:iframe.windowRight-btnPos.right,left:btnPos.left-iframe.winScrollLeft,bottom:iframe.windowBottom-btnPos.top,top:btnPos.bottom-iframe.winScrollTop};var align=iframe.__align;if(!iframe.__align){align={horiz:1,vert:1};if((newSize.width>space.right)&&(space.left>space.right)){if((newSize.width<=space.left)||iframe.__constrainSize){align.horiz=-1;}}if((newSize.height>space.bottom)&&(space.top>space.bottom)){if((newSize.height<=space.top)||iframe.__constrainSize){align.vert=-1;
}}iframe.__align=align;}if(iframe.__constrainSize){var max=(align.horiz==1)?space.right:space.left;if(newSize.width>max){newSize.width=max;iframe.style.width=(newSize.width-iframe.__horizCorrection)+"px";}max=(align.vert==1)?space.bottom:space.top;if(newSize.height>max){newSize.height=max;iframe.style.height=(newSize.height-iframe.__vertCorrection)+"px";}}iframe.style.left=((align.horiz==1)?btnPos.right:(btnPos.left-newSize.width))+"px";iframe.style.top=((align.vert==1)?btnPos.top:(btnPos.bottom-newSize.height))+"px";
}}function stdPrompterIFrameLoaded(){var iframe=g_LANSAObjects.getObjectById("stdPrompterFrame");if(iframe&&iframe.__isclosing){return;}if(iframe.getContentDocLocation()=="about:blank"){return;}if(stdPrompterIsCloseWR(iframe)){stdPrompterExchangeFields(iframe);stdPrompterOnChange(iframe);}else{iframe.style.visibility="visible";iframe.onblur=null;removeEvent(iframe.getContentDoc(),"blur",stdPrompterIFrameLostFocus);iframe.focus();if(iframe.contentWindow){iframe.contentWindow.focus();}window.setTimeout("stdPrompterSetOnBlur();",1);
...
var WAMName=stdPrompterGetProperty(iframe,"on_change_wamname");var WRName=stdPrompterGetProperty(iframe,"on_change_wrname");var protocol=stdPrompterGetProperty(iframe,"on_change_protocol");var target=stdPrompterGetProperty(iframe,"on_change_target_window_name");var reentryField=stdPrompterGetProperty(iframe,"on_change_reentryfield");var reentryValue=stdPrompterGetProperty(iframe,"on_change_reentryvalue");var theBtn=iframe.__theBtn;stdPrompterIFrameClose(iframe);if(WAMName!=""&&WRName!=""){var f=(!formName)?getParentForm(theBtn):createStdHiddenForm(WAMName,WRName,formName,protocol,target);
if(f!=null){if(reentryField!=""){insertHiddenField(f,reentryField,reentryValue);}submitForm(f,WAMName,WRName,target,protocol);}}}function stdPrompterIFrameClose(iframe){if(!iframe){iframe=g_LANSAObjects.getObjectById("stdPrompterFrame");}if(iframe!=null){if(iframe.__closeMeTO){clearTimeout(iframe.__closeMeTO);iframe.__closeMeTO=null;}iframe.__theBtn=null;iframe.__propmterInfo=new Object();iframe.style.visibility="hidden";iframe.style.height="0px";iframe.style.width="0px";var d=iframe.getContentDoc();
iframe.__isclosing=true;d.close();d.open();d.close();}}function stdPrompterIFrameLostFocus(){var iframe=g_LANSAObjects.getObjectById("stdPrompterFrame");if(iframe.style.visibility!="hidden"){if(!iframe.__closeMeTO){iframe.__closeMeTO=setTimeout("stdPrompterIFrameClose()",200);}}}Lstd.Weblets.stdSplitter=function(){};Lstd.Weblets.stdSplitter.addResizeCheck=function(splitter){if(!this._splitters){this._splitters=[];}this._splitters.push(splitter);splitter._lastSize={w:splitter.element.offsetWidth,h:splitter.element.offsetHeight};
this._resizeCheck();};Lstd.Weblets.stdSplitter._resizeCheck=function(){if(!this._splitters||this._timeoutID){return;}var self=this;this._timeoutID=window.setTimeout(function(){for(var i=0,s;s=self._splitters[i];i++){var newSize={w:s.element.offsetWidth,h:s.element.offsetHeight};if((newSize.w!=s._lastSize.w)||(newSize.h!=s._lastSize.h)){s.resize();s._lastSize=newSize;}}self._timeoutID=0;self._resizeCheck();},250);};Lstd.Weblets.stdSplitter.prototype.init=function(splitterID,splitterData){this.div1Size=splitterData.panel1Size||20;
this.element=document.getElementById(splitterID);this.div1=this.element.childNodes[0];this.divider=this.element.childNodes[1];this.div2=this.element.childNodes[2];this.type=splitterData.type;this.styleSizeProp=this.type=="v"?"width":"height";this.stylePosProp=this.type=="v"?"left":"top";this.offsetProp=this.type=="v"?"offsetWidth":"offsetHeight";this.clientProp=this.type=="v"?"clientWidth":"clientHeight";if(!Lstd.Utils.boxModel()){this.element.parentNode.style.overflow="hidden";}this.divider.style.display="block";
this.resize();var self=this;addEvent(this.divider,"mousedown",function(e){self.onMouseDown(e);},false);Lstd.Weblets.stdSplitter.addResizeCheck(this);};Lstd.Weblets.stdSplitter.prototype.resize=function(){var total=this.element[this.clientProp];this.div1.style[this.styleSizeProp]=(total*this.div1Size/100)+"px";this.divider.style[this.stylePosProp]=this.div1[this.offsetProp]+"px";this.div2.style[this.styleSizeProp]=(total-this.div1[this.offsetProp]-this.divider[this.offsetProp])+"px";this.div2.style[this.stylePosProp]=(this.div1[this.offsetProp]+this.divider[this.offsetProp])+"px";
this.div1.firstChild.style.width=this.div1.clientWidth+"px";this.div1.firstChild.style.height=this.div1.clientHeight+"px";this.div2.firstChild.style.width=this.div2.clientWidth+"px";this.div2.firstChild.style.height=this.div2.clientHeight+"px";};Lstd.Weblets.stdSplitter.prototype.onMouseDown=function(e){this.endDrag();this._mouseOffset=(this.type=="v")?getRelativeMouseX(this.divider,e):getRelativeMouseY(this.divider,e);var spCover=document.getElementById("StdSplitterDragBar");if(spCover){spCover.parentNode.removeChild(spCover);
}var spCover=document.createElement("div");spCover.id="StdSplitterDragCover";var s=spCover.style;s.cursor=getCurrentStyle(this.divider,"cursor");var spBar=document.createElement("div");spBar.appendChild(document.createComment(""));spBar.id="StdSplitterDragBar";s=spBar.style;s.cursor=getCurrentStyle(this.divider,"cursor");s.position="absolute";s.left=this.divider.offsetLeft+"px";s.top=this.divider.offsetTop+"px";s.width=this.divider.offsetWidth+"px";s.height=this.divider.offsetHeight+"px";var spCover2=document.createElement("div");
spCover2.appendChild(document.createComment(""));spCover2.id="StdSplitterDragCover2";spCover.appendChild(spBar);spCover.appendChild(spCover2);this.element.appendChild(spCover);this.dragBar=spBar;this.dragCover=spCover;var self=this;this._onMouseMove=function(e){self.onMouseMove(e);};this._onMouseUp=function(e){self.onMouseUp(e);};if(document.addEventListener){document.addEventListener("mousemove",self._onMouseMove,true);document.addEventListener("mouseup",self._onMouseUp,true);}else{spBar.attachEvent("onmousemove",self._onMouseMove);
spBar.attachEvent("onmouseup",self._onMouseUp);if(spBar.setCapture){spBar.setCapture();}}cancelEvent(e);return false;};Lstd.Weblets.stdSplitter.prototype.onMouseMove=function(e){var p=this.dragCover,db=this.dragBar,max=p[this.clientProp]-db[this.offsetProp];var newPos=((this.type=="v")?getRelativeMouseX(p,e):getRelativeMouseY(p,e))-this._mouseOffset;if(newPos<0){newPos=0;}if(newPos>max){newPos=max;}db.style[this.stylePosProp]=newPos+"px";cancelEvent(e);return false;};Lstd.Weblets.stdSplitter.prototype.onMouseUp=function(e){var s=this.styleSizeProp,c=this.clientProp,o=this.offsetProp,delta=((this.type=="v")?getRelativeMouseX(this.divider,e):getRelativeMouseY(this.divider,e))-this._mouseOffset,size1=this.div1[o]+delta,size2=this.div2[o]-delta;
if(size1<0){size2+=size1;size1=0;}else{if(size2<0){size1+=size2;size2=0;}}this.div1.style[s]=size1+"px";this.div2.style[s]=size2+"px";this.div1.firstChild.style[s]=this.div1[c]+"px";this.div2.firstChild.style[s]=this.div2[c]+"px";this.div1Size=size1/this.element[this.clientProp]*100;this.divider.style[this.stylePosProp]=this.div1[this.offsetProp]+"px";this.div2.style[this.stylePosProp]=(this.div1[this.offsetProp]+this.divider[this.offsetProp])+"px";this.endDrag();cancelEvent(e);return false;};Lstd.Weblets.stdSplitter.prototype.endDrag=function(e){if(this.dragCover){if(document.removeEventListener){document.removeEventListener("mousemove",this._onMouseMove,true);
...
}else{return;}}var btn=getEventTarget(e);if(btn==null){return;}if(isDblClick(btn)){return false;}var f=(!formName)?getParentForm(btn):createStdHiddenForm(WAMName,WRName,formName,protocol,"");if(f!=null){insertHiddenField(f,reentryField,btnType);insertHiddenField(f,prevCondFld,prevCondVal);insertHiddenField(f,nextCondFld,nextCondVal);insertHiddenField(f,pageCountFld,pageCountVal);Lstd.Fields.check4VF_WAMEVENT(f,btn);submitForm(f,WAMName,WRName,"",protocol);}cancelEvent(e);return false;}Lstd.Weblets.stdListPagingImages={_initDone:false,init:function(id,data){var slpi=Lstd.Weblets.stdListPagingImages;
if(!slpi._initDone&&window.jQuery){$(".stdListPagingButtons img").hover(function(){this.src=this.src.replace(/\/normal\//,"/hot/");},function(){this.src=this.src.replace(/\/hot\//,"/normal/");});}slpi._initDone=true;}};function stdNavPanel2FrameInfo(iframeName,formname,wamName,wrName,protocol,async,sizeToContent,sizeToContentAxis,width,height,waitContentImg,waitContentImgAlignment,waitContentImgClass,waitContentClass,waitContentTimeout){this.name=iframeName;this.formname=formname;this.id=formname+"_"+iframeName;
this.iframe=null;this.wamName=wamName;this.wrName=wrName;this.protocol=protocol;this.async=async;this.sizeToContent=sizeToContent;this.sizeToContentAxis=sizeToContentAxis;this.width=width;this.height=height;this.waitContentImg=waitContentImg;this.waitContentImgAlignment=waitContentImgAlignment;this.waitContentImgClass=waitContentImgClass;this.waitContentClass=waitContentClass;this.waitContentTimeout=waitContentTimeout;this.url=null;this.waitContent=null;this.loaded=false;this.loadQueued=false;this.reloadQueued=false;
this.sized=false;this.waitImgLoaded=false;this.waitContentDone=false;var ifo=this;this.getFrame=function(){if(ifo.iframe==null){ifo.iframe=document.getElementById(ifo.id);}return ifo.iframe;};this.getDocument=function(){var theFrame=ifo.getFrame();return(theFrame.contentDocument)?theFrame.contentDocument:theFrame.contentWindow.document;};this.getUrl=function(){if(ifo.url==null){ifo.url=getHiddenSpanContent(ifo.getFrame().parentNode,"nav_url");}return ifo.url;};this.getWaitContent=function(){if(ifo.waitContent==null){ifo.waitContent=getHiddenSpanContent(ifo.getFrame().parentNode,"wait_content");
