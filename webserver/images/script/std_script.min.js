...
this.getMessageText=function(messageId){var strings=Lstd.I18n._strings;var theMsg=(strings)?(strings[messageId]||""):"";return(arguments.length>1)?Lstd.Utils.strargs(theMsg,Array.prototype.slice.call(arguments,1,arguments.length)):theMsg;};this.getMessageTextArr=function(messageId){var arrays=Lstd.I18n._arrays;return(arrays)?(arrays[messageId]||null):null;};g_StdLocaleMgr=this;}function StdMessageMgr(languageId){new StdLocaleMgr(languageId,false);}function StdNumberParser(dec_sep){var expr=dec_sep=="."?"[^\\d.]":dec_sep==","?"[^\\d,]":dec_sep=="e"?"(\\+|-)\\d+\\.\\d{1,16}E\\+\\d{3}":"[\\D]";
this.regExp=new RegExp(expr,"g");this.negValueExp=new RegExp("(^-)|((-|CR)$)");this.isCommaSep=dec_sep==",";this.parseInt=StdNumberParser_parseInt;this.parseDecimal=StdNumberParser_parseDecimal;this.parseFloat=StdNumberParser_parseFloat;}function StdNumberParser_parseInt(str){var isNegative=str.search(this.negValueExp)!=-1;var intStr=str.replace(this.regExp,"");var intValue=intStr.length>0?parseInt(intStr,10):0;return isNegative?-intValue:intValue;}function StdNumberParser_parseDecimal(str){var isNegative=str.search(this.negValueExp)!=-1;
var floatStr=str.replace(this.regExp,"");var floatValue=floatStr.length>0?parseFloat(this.isCommaSep?floatStr.replace(",","."):floatStr):0;return isNegative?-floatValue:floatValue;}function StdNumberParser_parseFloat(str){var isValidFloat=str.search(this.regExp)==0;return isValidFloat?parseFloat(str):0;}function SetValueCase(oInput,propertyName){if(oInput==null){return;}if(propertyName=="value"){var textTransform=oInput.currentStyle.textTransform;if(textTransform=="uppercase"){var value=oInput.value;
var valueUpper=value.toUpperCase();if(value!=valueUpper){oInput.value=valueUpper;}}else{if(textTransform=="lowercase"){var value=oInput.value;var valueLow=value.toLowerCase();if(valueLow!=value){oInput.value=valueLow;}}}}}function _HandleEvent(WAM,WebRoutine,techServ,actionRequest,partition,language,optForm,optTarget,optProtocol,optSessionKey,optDebugMode){if(optForm==null){optForm=document.LANSA;}var prevAction=optForm.action;var prevTarget=optForm.target;var action="";if(optProtocol!=null&&optProtocol.length>0){action+=optProtocol;
if(optProtocol.search(new RegExp("//","g"))<0){action+="//"+stdGetOwnerDocument(optForm).location.host;}}action+=actionRequest+"?wam=";action+=WAM;action+="&webrtn=";action+=WebRoutine;action+="&ml="+techServ;action+="&part="+partition+"&lang="+language;if(optDebugMode!=null&&optDebugMode.length>0){action+="&debug="+optDebugMode;}if(optSessionKey!=null){action+="&sid="+optSessionKey;}optForm.action=action;optForm.target=(optTarget)?optTarget:"";optForm.submit();optForm.target=prevTarget;optForm.action=prevAction;
}function _HandleServiceEvent(ServiceName,techServ,actionRequest,partition,language,optForm,optTarget,optProtocol,optSessionKey,optDebugMode){var Form=optForm!=null?optForm:document.LANSA;var prevAction=Form.action;var prevTarget=Form.target;var action="";if(optProtocol!=null&&optProtocol.length>0){action+=optProtocol;if(optProtocol.search(new RegExp("//","g"))<0){action+="//"+stdGetOwnerDocument(Form).location.host;}}action+=actionRequest+"?srve="+ServiceName+"&part="+partition+"&lang="+language;
action+="&ml="+techServ;if(optDebugMode!=null&&optDebugMode.length>0){action+="&debug="+optDebugMode;}if(optSessionKey!=null){action+="&sid="+optSessionKey;}Form.action=action;Form.target=(optTarget)?optTarget:"";Form.submit();Form.target=prevTarget;Form.action=prevAction;}function _HandleEventTarget(Form,Target,techServ,actionRequest,partition,language,WebApp,WebRoutine,optProtocol,optSessionKey,optDebugMode){if(Form==null){Form=document.LANSA;}var prevTarget=Form.target;Form.target=(Target)?Target:"";
var prevAction=Form.action;var action="";if(optProtocol!=null&&optProtocol.length>0){action+=optProtocol;if(optProtocol.search(new RegExp("//","g"))<0){action+="//"+stdGetOwnerDocument(Form).location.host;}}action+=actionRequest+"?wam=";action+=WebApp;action+="&webrtn=";action+=WebRoutine;action+="&ml="+techServ;action+="&part="+partition+"&lang="+language;if(optDebugMode!=null&&optDebugMode.length>0){action+="&debug="+optDebugMode;}if(optSessionKey!=null){action+="&sid="+optSessionKey;}Form.action=action;
Form.submit();Form.target=prevTarget;Form.action=prevAction;}function _HandleDefaultSubmit(oForm){if(oForm==null){return false;}var oHandler=oForm.getAttribute("__defaultSubmitHandler");if(oHandler==null){return false;}return new Function("oForm",oHandler)(oForm);}function stdCreateTempForm(ownerDoc){var oTempForm=ownerDoc.createElement("form");if(oTempForm!=null){if(typeof oTempForm.setAttribute==="function"){oTempForm.setAttribute("method","post");}else{oTempForm=ownerDoc.createElement('<form method="post"></form>');
...
}}if(theObj.value!=value){theObj.value=value;if(theObj.id!=""){stdSetElementValue(theObj.id,value,true);}}}function stdSetElementValue(id,value,async){if(async){setTimeout("stdSetElementValue('"+id+"', '"+value+"', false)",1);}else{var elem=document.getElementById(id);if(elem!=null){elem.value=value;}}}function isValidDBCS(theObj,KeybdShift,EBCDICLength){var FieldValue=theObj.value;var returnString="";var totalDBCS=0;var totalSBCS=0;var totalLength=0;var inDBCS=false;var chkDBCSChar=false;for(var i=0;
i<FieldValue.length;i++){var ch=FieldValue.charAt(i);chkDBCSChar=isDBCSChar(ch);if(chkDBCSChar){if(!inDBCS){inDBCS=true;totalLength+=2;}totalLength+=2;if((totalLength<=EBCDICLength)&&(KeybdShift!="A")){returnString+=ch;}totalDBCS++;}else{totalLength+=1;if((totalLength<=EBCDICLength)&&(KeybdShift!="J")){returnString+=ch;}if(inDBCS){inDBCS=false;}totalSBCS++;}}if(totalLength>EBCDICLength){alert(g_StdLocaleMgr.getMessageText("BadDBCS_TooLong",totalLength,EBCDICLength));if(theObj.id!=""){stdSetElementValue(theObj.id,returnString,true);
