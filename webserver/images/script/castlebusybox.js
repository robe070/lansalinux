...
//   msg - (optional) message to show in the busy box
//
...
//
function BusyBox(id, varName, imageCount, imageNamePrefix, imageNameSuffix, imageDelay, width, height, url, msg)
{
	// Initialize object
	this.id = id;
	...
	this.Width = width;
	this.Height = height;
	this.Msg = msg != null ? msg : "Processing";
	...
	if (this.IFrame == null) this.IFrame = this.CreateIFrame();
...
// CreateIFrame()
// This method creates the BusyBox IFrame if it doesn't exist
BusyBox.prototype.CreateIFrame = function()
{
...
BusyBox.prototype.RenderContent = function()
{
	...
	doc.open();
	doc.writeln("<html xmlns=\"http://www.w3.org/1999/xhtml\"><head><title></title></head><body ondragstart='return false;' style='margin:0px; background-color:white;'>");
	doc.writeln("<div id='" + this.DivID + "' align=center " + style + ">");
	doc.writeln("<img id='" + this.ImgID + "' src=''>");
	doc.writeln("<br /><h3>" + this.Msg + "</h3>");
...
BusyBox.prototype.Resize = function()
{
	...
	if( BusyBox.IsBrowserIE() )
	{
		...
		var body = frames[this.id].document.body;
		this.IFrame.style.width = body.scrollWidth + 7;
		this.IFrame.style.height = body.scrollHeight + 7;
	}
	else
	{
